...
class UberManager: NSObject {
    ...
    var counterStatus = 0
    func advanceStatusSandboxRequest(requestId: String) {
        if server!.hasOAuthToken() {
            // Sandbox functionality.
            let parameters = ["requestID": requestId, "status": mapStatus[counterStatus]]
            counterStatus += 1
    ...
    func createUberWrapper(endLat: CGFloat, endLong: CGFloat) {
        checkOAuth()
        while(!server!.hasOAuthToken()){}
        let startLat = CGFloat(37.87)
        let startLong = CGFloat(-122.25)
        let result = self.createUberRequest("a1111c8c-c720-46c3-8534-2fcdd730040d", startLat: startLat, startLong: startLong, endLat: endLat, endLong: endLong)
